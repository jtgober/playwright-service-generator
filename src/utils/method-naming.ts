 ...
 * Removes "ApiV1" from the final method name only when merge is false.
 */
export const toCamelCaseMethodName = (
  ...
  // Use operationId directly if provided, optionally removing ApiV1
  if (operationId) {
    return merge ? operationId : operationId.replace(/ApiV1/i, '');
  }
  ...
  // Remove "ApiV1" from the method name only when merge is false
  if (!merge) {
